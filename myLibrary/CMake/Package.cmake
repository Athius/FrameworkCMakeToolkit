...
macro(set_project_version VERSION_MAJOR VERSION_MINOR VERSION_PATCH)
    set_if_not_set(${LIBRARY_OUTPUT_NAME}_VERSION_MAJOR ${VERSION_MAJOR})
    set_if_not_set(${LIBRARY_OUTPUT_NAME}_VERSION_MINOR ${VERSION_MINOR})
    set_if_not_set(${LIBRARY_OUTPUT_NAME}_VERSION_PATCH ${VERSION_PATCH})
    set_if_not_set(${LIBRARY_OUTPUT_NAME}_VERSION "${${LIBRARY_OUTPUT_NAME}_VERSION_MAJOR}.${${LIBRARY_OUTPUT_NAME}_VERSION_MINOR}.${${LIBRARY_OUTPUT_NAME}_VERSION_PATCH}") 
endmacro()

macro(try_build_package)
    set_project_version(1 0 0)
            
    set(CPACK_PACKAGE_NAME "${LIBRARY_OUTPUT_NAME}")
    set(CPACK_PACKAGE_VENDOR "Romain LEGUAY")
    set(CPACK_PACKAGE_DESCRIPTION_SUMMARY "Example test framework")
    set(CPACK_PACKAGE_VERSION_MAJOR "${${LIBRARY_OUTPUT_NAME}_VERSION_MAJOR}")
    set(CPACK_PACKAGE_VERSION_MINOR "${${LIBRARY_OUTPUT_NAME}_VERSION_MINOR}")
    set(CPACK_PACKAGE_VERSION_PATCH "${${LIBRARY_OUTPUT_NAME}_VERSION_PATCH}")
    ...
    foreach(component ${COMPONENT_NAMES})
      list(APPEND CPACK_INSTALL_CMAKE_PROJECTS "${CMAKE_CURRENT_BINARY_DIR};${LIBRARY_OUTPUT_NAME};${component};/")
    endforeach()
    ...
    add_custom_target(${LIBRARY_OUTPUT_NAME}_package "cpack" "--config" "${CPACK_OUTPUT_CONFIG_FILE}")
    add_custom_target(${LIBRARY_OUTPUT_NAME}_package_source "cpack" "--config" "${CPACK_SOURCE_OUTPUT_CONFIG_FILE}")
